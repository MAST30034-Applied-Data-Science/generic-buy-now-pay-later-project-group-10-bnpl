...
first_transaction_date = transactions.select(
            first("order_datetime").alias('date'))
first_transaction_year = first_transaction_date.withColumn("year", 
            year(col('date')))

last_transaction_date = transactions.select(
            last("order_datetime").alias('date'))
...
# Specify the url
SA2_URL_ZIP = "https://www.abs.gov.au/statistics/standards/australianstatistical-geography-standard-asgs-edition-3/jul2021-jun2026/access-and-downloads/digital-boundary-files/SA2_2021_AUST_SHP_GDA2020.zip"
...
population = pd.read_excel("../data/SA2_total_population/SA2_pop.xlsx",
        sheet_name="Table 1")
...
new_columns = ['SA2_code', 'SA2_name', 'state_code', 'state_name', 
        'geometry']
...
#==============================================================================
# Loading the data
income = pd.read_excel("../data/SA2_income/SA2_income.xlsx", 
...
postcodes = postcodes.select(postcodes.postcode.alias("postcodes"), 
...
SA2_datasets = pd.merge(income_census, population, 
